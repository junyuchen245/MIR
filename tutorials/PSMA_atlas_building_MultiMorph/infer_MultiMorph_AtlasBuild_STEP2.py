...
def main():
    ...
    model.cuda()
    best_model = torch.load('/scratch/jchen/python_projects/custom_packages/MIR/tutorials/PSMA_atlas_building_MultiMorph/experiments/MultiMorph_AutoPET_SSIM_1_diffusion_1/ssim0.8313.pth.tar')['state_dict']
    ...
    val_loader = DataLoader(val_set, batch_size=25, shuffle=False, num_workers=4, pin_memory=True, drop_last=True)
    '''
    Validation
    '''
    with torch.no_grad():
        for data in val_loader:
            ...
            print(ct_atlas.shape, suv_atlas.shape)
            save_atlas(ct_atlas, 'atlas/ct/' + save_dir, filename='final.nii.gz')
            save_atlas(suv_atlas, 'atlas/suv/' + save_dir, filename='final.nii.gz')
            sys.exit()
