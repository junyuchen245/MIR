...
from MIR.models import VFA, SpatialTransformer, AffineReg3D
...
import random
from MIR.models import fit_warp_to_svf, fit_warp_to_svf_fast
...
def ct_norm(img_npy: np.ndarray) -> np.ndarray:
    ...
    img_min = np.percentile(img_npy, 92)
...
def main():
    ...
    tmp_nib = nib.load('LUMIR_template.nii.gz')
    ...
    for img_i in glob.glob(val_dir + '*_stripped.nii.gz'):
        ...
        fx_npy_ = img_nib.get_fdata()
        fx_npy = ct_norm(fx_npy_)
        ...
        mv_img = torch.from_numpy(mv_img[None, None, ...]).cuda().float()
        fx_img = torch.from_numpy(fx_npy[None, None, ...]).cuda().float()
        tmp_img = torch.from_numpy(tmp_img[None, None, ...]).cuda().float()
        affine_model = AffineReg3D(vol_shape=tmp_img.shape[2:], dof = "affine", scales = (0.5, 1), loss_funcs = ("nmi", "nmi")).cuda(0)
        mv_aff_out = affine_model.optimize(mv_img, tmp_img, steps_per_scale=(30, 30), verbose=True, optimizer_name='adam', mind_init=True, lr=1e-3)
        fx_aff_out = affine_model.optimize(fx_img, tmp_img, steps_per_scale=(30, 30), verbose=True, optimizer_name='adam', mind_init=True, lr=1e-3)
        mv_aff_img = mv_aff_out['warped']
        fx_aff_img = fx_aff_out['warped']
        
        plt.figure(figsize=(12, 6))
        plt.subplot(2, 3, 1)
        plt.imshow(mv_img[0, 0, :, :, D//2].detach().cpu().numpy(), cmap='gray')
        plt.title('Moving Image')
        plt.axis('off')

        plt.subplot(2, 3, 2)
        plt.imshow(tmp_img[0, 0, :, :, D//2].detach().cpu().numpy(), cmap='gray')
        plt.title('Fixed Image')
        plt.axis('off')

        plt.subplot(2, 3, 3)
        plt.imshow(mv_aff_img[0, 0, :, :, D//2].detach().cpu().numpy(), cmap='gray')
        plt.title('Deformed Moving Image')
        plt.axis('off')
        
        plt.subplot(2, 3, 4)
        plt.imshow(fx_img[0, 0, :, :, D//2].detach().cpu().numpy(), cmap='gray')
        plt.title('Moving Image')
        plt.axis('off')
        ...
        plt.subplot(2, 3, 6)
        plt.imshow(fx_aff_img[0, 0, :, :, D//2].detach().cpu().numpy(), cmap='gray')
        plt.title('Deformed Moving Image')
        plt.axis('off')

        plt.savefig('tmp_aff.png')
        
        
        print(mv_aff_img.shape, fx_aff_img.shape)
        with torch.no_grad():
            flow = model((mv_aff_img, fx_aff_img))
        deformable_flow = fit_warp_to_svf_fast(flow, nb_steps=7, iters=200, lr=0.1, output_type='svf')
        
        affine_flow = mv_aff_out['flow']
        flow = deformable_flow + reg_model(affine_flow, deformable_flow)
        mv_img = affine_model._pad_or_crop(mv_img, tmp_img.shape[2:])
        fx_img = affine_model._pad_or_crop(fx_img, tmp_img.shape[2:])
        ...
        plt.subplot(1, 3, 3)
        plt.imshow(dmv_img[0, 0, :, :, D//2].detach().cpu().numpy(), cmap='gray')
        plt.title('Deformed Moving Image')
        plt.axis('off')
