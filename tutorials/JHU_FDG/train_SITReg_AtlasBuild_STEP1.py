...
from MIR.models import TemplateCreation, SpatialTransformer
...
def create_model(INPUT_SHAPE = (160, 192, 224)) -> SITReg:
...
class Hecktor25Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        x_suv = nib.load(self.path + '{}_PET.nii.gz'.format(mov_name))
...
def main():
    ...
    train_dir = '/scratch2/jchen/DATA/HECKTOR25/preprocessed/'
    val_dir = '/scratch2/jchen/DATA/HECKTOR25/preprocessed/'
    save_dir = 'SITRegAtlas_JHU_SSIM_{}_MS_{}_diffusion_{}/'.format(weights[0], weights[1], weights[2])
    ...
    '''
    H, W, D = 192, 192, 144
    sitreg_model = create_model(INPUT_SHAPE=(H, W, D))
    model = TemplateCreation(sitreg_model, (H, W, D), use_sitreg=True)
    model.cuda()
    ...
    Initialize training
    '''
    full_names = glob.glob(train_dir + '*_CTSeg*')
    full_names = [name.split('/')[-1].split('_CTSeg')[0] for name in full_names]
    ...
    train_set = Hecktor25Dataset(train_dir, train_names)
    val_set = Hecktor25Dataset(val_dir, val_names)
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            x_suv_def = spatial_trans(x_suv.float(), pos_flow.float())
            y_suv_def = spatial_trans(y_suv.float(), neg_flow.float())
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                def_out = spatial_trans(x.cuda().float(), pos_flow.cuda())
                def_grid = spatial_trans(grid_img.float(), pos_flow.cuda())
