...
from MIR.models import convex_adam_MIND_SPR, SpatialTransformer
...
def objective(trial):
    ...
    config.logBeta_weight = trial.suggest_float("wt_logBeta", 0.001, 0.1)
    config.lambda_weight = trial.suggest_float("wt_localGrad", 0.8, 3.)
    ...
    print('LogBeta weight: {}, LocalGrad weight: {}, Grid SP: {}, Disp HW: {}, N_iter: {}'.format(config.logBeta_weight, config.lambda_weight, config.grid_sp, config.disp_hw, config.selected_niter))
    # Validation of the model.
    eval_dsc = AverageMeter()
    for idx, data in enumerate(val_loader):
        y = data[0].cuda().float()
        y_seg = data[2].cuda().float()
        pos_flow, wts = model(x, y, config)
