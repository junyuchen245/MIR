...
from MIR.image_similarity import SSIM3D, DiceLoss
from MIR.deformation_regularizer import Grad3d
from MIR.utils import Logger, AverageMeter
from MIR.label_reference import remap_totalsegmentator_lbls
...
class JHUPSMADataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        x_seg = remap_totalsegmentator_lbls(x_seg, total_seg_version='v2', label_scheme='40lbls')
...
def one_hot_img(img, C):
    img_oh = nn.functional.one_hot(img.long(), num_classes=C)
    img_oh = torch.squeeze(img_oh, 1)
    img_oh = img_oh.permute(0, 4, 1, 2, 3).contiguous()
    return img_oh
...
def estimate_initial_template(train_loader):
    ...
    with torch.no_grad():
        ...
        for data in train_loader:
            x = data['CT'].cuda().float()
            x_suv = data['SUV'].cuda().float()
            x_mean += x
            x_suv_mean += x_suv
            idx += 1
...
def main():
    ...
    save_dir = 'VFAAtlas_PSMA_SSIM_{}_MS_{}_DSC_1_diffusion_{}/'.format(weights[0], weights[1], weights[2])
    ...
    criterion_dsc_ct = DiceLoss(num_class=40, one_hot=False)
    criterion_dsc_suv = DiceLoss(num_class=14, one_hot=False)
    SSIM = SSIM3D()
    best_ssim = 0
    writer = SummaryWriter(log_dir='logs/'+save_dir)
    # Exponential moving average (EMA) for one-hot labels
    # decay close to 1.0 -> slow update (more smoothing)
    ema_decay = 0.9
    ct_lbl_oh_ema = None
    suv_lbl_oh_ema = None
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            with torch.no_grad():
                ...
                ct_lbl = data['CT_seg'].cuda().float()
                suv_lbl = data['SUV_seg'].cuda().float()
                # compute one-hot encodings as float tensors
                ct_lbl_oh = one_hot_img(ct_lbl, C=40).float()
                suv_lbl_oh = one_hot_img(suv_lbl, C=14).float()
            ...
            ct_lbl_def = model.reg_model.spatial_trans(ct_lbl_oh.float(), neg_flow.float())
            suv_lbl_def = model.reg_model.spatial_trans(suv_lbl_oh.float(), neg_flow.float())
            # initialize EMA buffers on first use, then update
            if ct_lbl_oh_ema is None:
                ct_lbl_oh_ema = ct_lbl_def.clone().detach()
                suv_lbl_oh_ema = suv_lbl_def.clone().detach()
                
            loss_dsc_ct = criterion_dsc_ct(ct_lbl_oh_ema, ct_lbl_def)/2
            loss_dsc_suv = criterion_dsc_suv(suv_lbl_oh_ema, suv_lbl_def)/2
            ...
            loss = (loss_img_1 + loss_img_2 + loss_suv_1 + loss_suv_2)/2/2 + loss_mean_stream + loss_reg + (loss_dsc_ct + loss_dsc_suv)/2
            loss_img = loss_img_1 + loss_img_2
            loss_seg = loss_dsc_ct + loss_dsc_suv
            ...
            loss.backward()
            optimizer.step()
            # update EMA of one-hot labels
            with torch.no_grad():
                ct_lbl_oh_ema.mul_(ema_decay).add_(ct_lbl_def.detach() * (1.0 - ema_decay))
                suv_lbl_oh_ema.mul_(ema_decay).add_(suv_lbl_def.detach() * (1.0 - ema_decay))
            print('Iter {} of {} loss {:.4f}, Img Sim: {:.6f}, Suv Sim: {:.6f}, Seg: {:.6f}, Reg: {:.6f}'.format(idx, len(train_loader),
                                                                                            loss.item(),
                                                                                            loss_img.item(),
                                                                                            loss_suv.item(),
                                                                                            loss_seg.item(),
