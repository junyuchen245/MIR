...
def main():
    ...
    save_dir = f'VFASPR_wt_logBeta_{wt_logBeta}_ncc_{wt_ncc}_localGrad_{wt_localGrad}/'
    ...
    template_model = TemplateCreation(vfa_model, (H, W, D))
    best_model = torch.load('/scratch/jchen/python_projects/custom_packages/MIR/tutorials/PSMA_atlas_building/experiments/VFAAtlas_SSIM_1_MS_1_diffusion_1/ssim0.9367.pth.tar')
    template_model.load_state_dict(best_model['state_dict'])
    save_checkpoint({
            'epoch': best_model['epoch'],
            'state_dict': template_model.reg_model.state_dict(),
            'optimizer': best_model['optimizer'],
        }, save_dir='experiments/VFAAtlas_SSIM_1_MS_1_diffusion_1/', filename='vfa_ssim0.9367.pth.tar')
    sys.exit()
    '''
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            _, _, pos_flow, neg_flow, wts = model((x_in, y_in))
            ...
            print('Iter {} of {} loss {:.4f}, Img Sim: {:.6f}, Wts Reg: {:.6f}, Flow Reg: {:.6f}, Scale: {:.6f}'.format(idx, len(train_loader),
                                                                                                        loss.item(),
                                                                                                        loss_img.item(),
                                                                                                        loss_reg.item(),
                                                                                                        loss_reg2.item(),
