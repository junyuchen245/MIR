...
def main():
    ...
    rib_labels = [92, 93, 94, 95, 96, 97, 98,   # left ribs 1-12
         99, 100, 101, 102, 103,
    ...
    Validation
    '''
    ct_prob_atlas = torch.zeros((1, 118, H, W, D)).cuda()
    with torch.no_grad():
        for data in val_loader:
            ...
            y_seg_oh = nn.functional.one_hot(y_seg.long(), num_classes=118)
            ...
            del def_atlas, def_image, pos_flow, neg_flow, wts
            torch.cuda.empty_cache()
            
        ct_prob_atlas = ct_prob_atlas / len(val_loader)
        for lbl in rib_labels:
            ct_prob_atlas[:, lbl, ...] = ct_prob_atlas[:, lbl, ...]*6 #enhance ribs
        ...
        nib.save(ct_seg_atlas, 'atlas/seg/ct_seg_atlas_w_reg_118lbls.nii.gz')
