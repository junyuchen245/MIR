...
def main():
    ...
    save_dir = 'VFAAtlas_SSIM_{}_MS_{}_diffusion_{}/'.format(weights[0], weights[1], weights[2])
    ...
    ct_atlas_dir = 'atlas/ct/' + save_dir
    pt_atlas_dir = 'atlas/suv/' + save_dir
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            def_atlas, def_image, pos_flow, neg_flow, mean_stream = model((x_ct, y))
            
            def_ct_seg = model.reg_model.spatial_trans(y_seg_oh.cuda().float(), neg_flow.cuda())
            ct_prob_atlas = ct_prob_atlas + def_ct_seg

            def_suv_seg = model.reg_model.spatial_trans(y_suv_seg_oh.cuda().float(), neg_flow.cuda())
        ...
        nib.save(ct_seg_atlas, 'atlas/seg/ct_seg_atlas_40lbls.nii.gz')
        suv_seg_atlas = nib.Nifti1Image(suv_seg.float().detach().cpu().numpy()[0, 0], affine=affine)
        nib.save(suv_seg_atlas, 'atlas/seg/suv_seg_atlas_14lbls.nii.gz')
