//Modified to save the 3D displacement as a .nii.gz file
//Modified by: Junyu Chen
...
int main (int argc, char * const argv[]) {
	...
	float* ddf=new float[sz*3];
	float* warped=new float[sz];
	float* gx=new float[sz];
	float* gy=new float[sz];
	float* gz=new float[sz];
	for(int i=0;i<sz;i++){
    ...
    //warpAffineF(warped,im1,im1b,X,ux,vx,wx,gx,gy,gz);
    //gzWriteNifti("dense_disp_gx.nii.gz",gx,header,m,n,o,1);
    //gzWriteNifti("dense_disp_gy.nii.gz",gy,header,m,n,o,1);
    //gzWriteNifti("dense_disp_gz.nii.gz",gz,header,m,n,o,1);
