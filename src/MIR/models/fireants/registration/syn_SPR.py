...
from MIR.models.fireants.io.image import BatchedImages, FakeBatchedImages
from torch.optim import SGD, Adam
from torch.nn import functional as F
from MIR.models.fireants.utils.globals import MIN_IMG_SIZE
from MIR.models.fireants.registration.abstract import AbstractRegistration
from MIR.models.fireants.registration.deformation.compositive import CompositiveWarp
from MIR.models.fireants.registration.deformation.svf import StationaryVelocity
from MIR.models.fireants.losses.cc import gaussian_1d, separable_filtering
from MIR.models.fireants.utils.imageutils import downsample
from MIR.models.fireants.utils.util import compose_warp
from MIR.models.fireants.utils.warputils import compositive_warp_inverse
from MIR.models.fireants.interpolator import fireants_interpolator
from MIR.models.fireants.registration.deformablemixin import DeformableMixin
...
class SyNRegistration(AbstractRegistration, DeformableMixin):
    ...
    Args:
        ...
        displacement_reg (Optional[Union[Callable, nn.Module]], optional): Regularization on displacement field.
            Defaults to None.
        sim_weight (float, optional): Weight for similarity loss term. Defaults to 1.0.
        spr_reg (bool, optional): Enable spatially varying regularization on displacement field.
            Defaults to False.
        spr_lambda (float, optional): Weight for spatially varying displacement regularization.
            Defaults to 1.0.
        spr_logbeta_weight (float, optional): Log-beta weight regularizer for spatial weights.
            Defaults to 0.01.
        spr_lr (float, optional): Learning rate for spatial weight optimizer.
            Defaults to 0.05.
        spr_init (float, optional): Initial value for spatial weights.
            Defaults to 1.0.
        spr_weight_floor (float, optional): Small positive floor for spatial weights.
            Defaults to 1e-6.
    ...
    def __init__(self, scales: List[int], iterations: List[float], 
                ...
                spr_reg: bool = False,
                spr_lambda: float = 1.0,
                spr_logbeta_weight: float = 0.01,
                spr_lr: float = 0.05,
                spr_init: float = 1.0,
                spr_weight_floor: float = 1e-6,
                blur: bool = True,
        ...
        self.spr_reg = spr_reg
        self.spr_lambda = spr_lambda
        self.spr_logbeta_weight = spr_logbeta_weight
        self.spr_lr = spr_lr
        self.spr_init = spr_init
        self.spr_weight_floor = spr_weight_floor
        self.spr_wt_param = None
        self.spr_optimizer = None
    ...
    def _init_spr_weight(self, size_down):
        expected_shape = [self.opt_size, 1, *size_down]
        if self.spr_wt_param is None or list(self.spr_wt_param.shape) != expected_shape:
            if self.spr_wt_param is None:
                init_param = torch.full(expected_shape, _inv_softplus_value(self.spr_init),
                                        device=self.fixed_images.device, dtype=self.dtype)
            else:
                mode = 'bilinear' if self.dims == 2 else 'trilinear'
                with torch.no_grad():
                    wt = F.softplus(self.spr_wt_param).detach()
                    wt = F.interpolate(wt, size=size_down, mode=mode, align_corners=True)
                init_param = _inv_softplus_tensor(wt)
            self.spr_wt_param = nn.Parameter(init_param)
            self.spr_optimizer = Adam([self.spr_wt_param], lr=self.spr_lr)
    ...
    def optimize(self):
        ...
        for scale, iters in zip(self.scales, self.iterations):
            ...
            if self.spr_reg:
                self._init_spr_weight(size_down)
            ...
            for i in pbar:
                ...
                # smooth if required
                if self.smooth_warp_sigma > 0 and not self.spr_reg:
                    fwd_warp_field = separable_filtering(fwd_warp_field.permute(*self.fwd_warp.permute_vtoimg), warp_gaussian).permute(*self.fwd_warp.permute_imgtov)
                ...
                # compute loss
                loss = self.loss_fn(moved_image_warp, fixed_image_warp) * self.sim_weight
                loss_sim = loss.item()
                # add regularization
                ...
                spr_reg_loss = 0.0
                if self.spr_reg:
                    spr_wt = (self.spr_wt_param - self.spr_wt_param.min()) / (self.spr_wt_param.max() - self.spr_wt_param.min() + 1e-6)
                    spr_reg_loss = self.spr_lambda * (
                        spatial_grad_reg(fwd_warp_field, spr_wt) + spatial_grad_reg(rev_warp_field, spr_wt)
                    )
                    if self.spr_logbeta_weight > 0:
                        spr_reg_loss = spr_reg_loss + log_beta(spr_wt, 1.0 + self.spr_logbeta_weight)
                    loss = loss + spr_reg_loss
                # backward
                loss.backward()
                if self.progress_bar:
                    pbar.set_description("scale: {}, iter: {}/{}, loss: {:4f}, spr_loss: {:4f}".format(scale, i, iters, loss_sim/scale_factor, spr_reg_loss.item()/scale_factor if self.spr_reg else 0.0))
...
if __name__ == '__main__':
    ...
    from MIR.models.fireants.utils.util import get_gpu_memory
...
def log_beta(weight_volume, alpha, eps=1e-6):
    ...
    return (1.0 - alpha) * beta.mean()


def spatial_grad_reg(warp_field: torch.Tensor, weight_volume: torch.Tensor) -> torch.Tensor:
    """Spatially weighted gradient regularization for displacement field.

    Args:
        warp_field: Displacement field, shape [B, H, W, [D], dims].
        weight_volume: Spatial weights, shape [B, 1, H, W, [D]].
    """
    if warp_field.dim() == 4:
        # [B, H, W, 2]
        weight = weight_volume[:, 0]
        dy = warp_field[:, 1:, :, :] - warp_field[:, :-1, :, :]
        dx = warp_field[:, :, 1:, :] - warp_field[:, :, :-1, :]
        dy = dy.pow(2).sum(-1) * weight[:, 1:, :]
        dx = dx.pow(2).sum(-1) * weight[:, :, 1:]
        return (dx.mean() + dy.mean()) / 2.0
    elif warp_field.dim() == 5:
        # [B, H, W, D, 3]
        weight = weight_volume[:, 0]
        dy = warp_field[:, 1:, :, :, :] - warp_field[:, :-1, :, :, :]
        dx = warp_field[:, :, 1:, :, :] - warp_field[:, :, :-1, :, :]
        dz = warp_field[:, :, :, 1:, :] - warp_field[:, :, :, :-1, :]
        dy = dy.pow(2).sum(-1) * weight[:, 1:, :, :]
        dx = dx.pow(2).sum(-1) * weight[:, :, 1:, :]
        dz = dz.pow(2).sum(-1) * weight[:, :, :, 1:]
        return (dx.mean() + dy.mean() + dz.mean()) / 3.0
    else:
        raise ValueError(f"Invalid warp_field shape: {warp_field.shape}")


def _inv_softplus_value(x: float, eps: float = 1e-6) -> float:
    return float(np.log(np.expm1(x) + eps))


def _inv_softplus_tensor(x: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
