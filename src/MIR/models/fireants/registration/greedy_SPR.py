    ...
    def optimize(self):
        ...
        for scale, iters in zip(self.scales, self.iterations):
            ...
            for i in pbar:
                ...
                loss = self.loss_fn(moved_image, fixed_image_down)*self.sim_weight
                loss_sim = loss.item()
                # apply regularization on the warp field
                if self.displacement_reg is not None:
                    loss = loss + self.displacement_reg(warp_field)
                if self.spr_reg:
                    spr_wt = (self.spr_wt_param-self.spr_wt_param.min())/(self.spr_wt_param.max()-self.spr_wt_param.min()+1e-6)  # normalize to [0,1]
                    spr_reg_loss = self.spr_lambda * spatial_grad_reg(warp_field, spr_wt)
...
def spatial_grad_reg(warp_field: torch.Tensor, weight_volume: torch.Tensor) -> torch.Tensor:
    ...
    if warp_field.dim() == 4:
        ...
        dy = dy.pow(2) * weight[:, 1:, :].unsqueeze(-1)
        dx = dx.pow(2) * weight[:, :, 1:].unsqueeze(-1)
        ...
        dy = dy.pow(2) * weight[:, 1:, :, :].unsqueeze(-1)
        dx = dx.pow(2) * weight[:, :, 1:, :].unsqueeze(-1)
        dz = dz.pow(2) * weight[:, :, :, 1:].unsqueeze(-1)
